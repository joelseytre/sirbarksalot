...
public class PlatformerCharacter2D : MonoBehaviour
    {
        ...
        private Transform m_CeilingCheck1;   // A position marking where to check for ceilings
        ...
        private void Awake()
        {
			...
			bcoll = gameObject.GetComponentInChildren<BoxCollider2D> ();
        }
        ...
        private void FixedUpdate()
        {
			...
			for (int i = 0; i < collidersg.Length; i++) {
				if (collidersg [i].gameObject != gameObject && collidersg[i].isTrigger==false) {
					m_Grounded = true;
				}
			}
			...
			// Deactivates boxcollider if crouched
			bool crouch = Input.GetKey(KeyCode.LeftShift);
			if (!crouch)
			{
				Collider2D[] collidersc = Physics2D.OverlapAreaAll (m_CeilingCheck1.position, m_CeilingCheck2.position);
				for (int i = 0; i < collidersc.Length; i++) 
				{
					if (collidersc [i].gameObject != gameObject && collidersc [i].isTrigger == false) {
						crouch = true;
					}
				}
			}
			bcoll.enabled = (!crouch || !m_Grounded);
        }
	...
	public void Grow(){
		...
		m_JumpForce *= 1400f/1200f;
	}

	public void Shrink(){
		gameObject.transform.localScale *= 0.5f;
		m_JumpForce *= 1200f/1400f;
	}
